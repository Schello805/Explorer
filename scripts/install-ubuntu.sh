...
MAIL_FROM="${MAIL_FROM:-info@schellenberger.biz}"
...
STRIPE_STARTER_YEARLY_PRICE_ID="${STRIPE_STARTER_YEARLY_PRICE_ID:-}"
STRIPE_PRO_MONTHLY_PRICE_ID="${STRIPE_PRO_MONTHLY_PRICE_ID:-}"
STRIPE_PRO_YEARLY_PRICE_ID="${STRIPE_PRO_YEARLY_PRICE_ID:-}"
STRIPE_SETUP_SERVICE_PRICE_ID="${STRIPE_SETUP_SERVICE_PRICE_ID:-}"
STRIPE_TAX_MODE="${STRIPE_TAX_MODE:-small_business_de}"
UPLOAD_MAX_MB="${UPLOAD_MAX_MB:-30}"
...
write_env_file() {
  ...
  if [[ "${INSTALL_POSTGRES}" == "true" ]]; then
    ...
    [[ -n "${DATABASE_URL}" ]] || fail "INSTALL_POSTGRES=false erfordert eine gesetzte DATABASE_URL."
  fi
  if [[ -z "${NEXT_PUBLIC_BASE_URL}" ]]; then
    NEXT_PUBLIC_BASE_URL="$(public_url)"
  fi
  if [[ -z "${MAIL_LOGO_URL}" ]]; then
    MAIL_LOGO_URL="${NEXT_PUBLIC_BASE_URL}/icons/platzguide-logo.png"
  fi
...
install_dependencies_and_build() {
  ...
  sudo -u "${APP_USER}" bash -lc "cd '${APP_DIR}' && npm ci --include=dev --no-audit --no-fund"
