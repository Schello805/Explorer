...
import { ArrowRight, Code2, ShieldCheck, Smartphone, TentTree } from "lucide-react";
...
export function PlatformLanding() {
  ...
  return <main className="min-h-screen bg-[#f5f2e9] text-[#18332b]">
    <section className="mx-auto grid min-h-screen w-[90%] content-center gap-6 py-8 lg:grid-cols-[1fr_.9fr] lg:items-center">
      <div>
        <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 text-xs font-bold text-[#195f4c] shadow-sm"><TentTree size={16} /> Entdecker Plattform</div>
      ...
      <form onSubmit={createInstance} className="rounded-[2rem] bg-white p-5 shadow-soft sm:p-7">
        ...
        <button disabled={loading} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#195f4c] px-5 py-3.5 font-bold text-white disabled:opacity-60">{loading ? "Wird erstellt …" : "Instanz erstellen"} <ArrowRight size={18} /></button>
      </form>
    </section>
  </main>;
}
