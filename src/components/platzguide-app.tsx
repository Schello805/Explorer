    ...
    <main>
      ...
      <section className="mx-auto w-[90%] py-3 sm:py-4">
        ...
        {mapConfigured && view === "map" ? (
          <CampMap tenant={tenant} stations={stations} onSelect={setSelected} />
...
function StationSheet({ station, favorite, checkinsEnabled, checkedIn, onFavorite, onCheckin, onClose }: { station: Station; favorite: boolean; checkinsEnabled: boolean; checkedIn: boolean; onFavorite: () => void; onCheckin: () => void; onClose: () => void }) {
  return <div className="fixed inset-0 z-40 flex items-end bg-black/35 p-3 sm:items-center sm:justify-center" onClick={onClose}><article onClick={(event) => event.stopPropagation()} className="animate-enter max-h-[82svh] w-full max-w-md overflow-auto rounded-[1.5rem] bg-white shadow-2xl sm:max-h-[90vh] sm:max-w-lg sm:rounded-[2rem]"><div className="relative h-32 sm:h-52" style={{ background: station.image }}><button onClick={onClose} aria-label="Stationsdetails schließen" className="absolute right-3 top-3 rounded-full bg-white p-2 shadow sm:right-4 sm:top-4"><X size={20} /></button></div><div className="p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-widest text-[var(--primary)] sm:text-xs">{statusLabel[station.status]}</p><h2 className="mt-1 break-words font-display text-2xl leading-tight sm:text-3xl">{station.name}</h2></div><button onClick={onFavorite} aria-label="Favorit" className="shrink-0 rounded-full border p-2.5 sm:p-3"><Heart size={22} fill={favorite ? "currentColor" : "none"} className={favorite ? "text-red-500" : ""} /></button></div><p className="mt-3 text-sm leading-6 text-[#18332b]/70 sm:mt-4 sm:text-base sm:leading-7">{station.description}</p><div className="mt-4 rounded-2xl bg-[var(--surface)] p-3 text-sm sm:mt-5 sm:p-4"><strong>Öffnungszeiten</strong><p className="mt-1 text-[#18332b]/65">{station.openingHours}</p></div><div className="mt-4 grid gap-2 sm:mt-5">{checkinsEnabled && <button onClick={onCheckin} disabled={checkedIn} className="rounded-xl border border-black/10 px-4 py-3 text-sm font-bold disabled:opacity-60 sm:px-5 sm:py-3.5 sm:text-base">{checkedIn ? "Bereits eingecheckt" : "Hier einchecken"}</button>}<a href={`https://www.openstreetmap.org/directions?to=${station.latitude},${station.longitude}`} target="_blank" rel="noreferrer" className="flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-3 text-sm font-bold text-white sm:px-5 sm:py-3.5 sm:text-base"><Navigation size={18} /> Navigation</a></div></div></article></div>;
}
