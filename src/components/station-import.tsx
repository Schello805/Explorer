...
export function StationImport({ tenantId, categories, onImport }: {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/35 p-4">
    <section className="max-h-full w-full max-w-5xl overflow-x-hidden overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
      ...
      <label className="mt-5 grid cursor-pointer place-items-center rounded-xl border border-dashed border-black/20 bg-[#f7f7f4] p-5 text-center"><FileSpreadsheet className="text-[#286551]" /><strong className="mt-2 text-sm">Datei auswählen</strong><span className="break-words text-xs text-black/40">Name, Kategorie, Breitengrad, Längengrad, Status, Öffnungszeiten</span><input title="CSV- oder XLSX-Datei mit Stationen auswählen. Vor dem Speichern wird eine Importvorschau angezeigt." type="file" accept=".csv,.xlsx" className="sr-only" onChange={(event) => event.target.files?.[0] && readFile(event.target.files[0])} /></label>
