...
import { Activity, Bell, BookOpen, CalendarDays, ChevronRight, CircleAlert, FileText, Gift, Globe2, ImageIcon, LayoutDashboard, MapPinned, Menu, MessageSquareWarning, Palette, Plus, Search, Settings, ShieldCheck, TentTree, Users, X } from "lucide-react";
...
const navigation = [
  ...
  { id: "tenants", label: "Campingplätze", icon: TentTree },
  ...
  { id: "rewards", label: "Entdecker-Pass", icon: Gift },
...
export function AdminConsole({ tenant, adminEmail }: { tenant: Tenant; adminEmail: string }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#f2f3ef] text-[#1b302a]">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-30 flex w-60 flex-col overflow-y-auto overflow-x-hidden bg-[#173c32] p-4 text-white transition-transform lg:translate-x-0", menuOpen ? "translate-x-0" : "-translate-x-full")}>
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#e8b65f] text-[#173c32]"><TentTree /></span><div><p className="font-display text-xl">Entdecker</p><p className="text-[10px] uppercase tracking-widest text-white/45">Plattform Admin</p></div></div><button className="lg:hidden" onClick={() => setMenuOpen(false)}><X /></button></div>
...
function Overview({ tenant, stationCount, onNavigate }: { tenant: Tenant; stationCount: number; onNavigate: (id: string) => void }) {
  return <div className="animate-enter"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-[#1b302a]/55">Guten Tag, Michael.</p><h2 className="mt-1 font-display text-4xl">Alles im grünen Bereich.</h2></div><p className="flex items-center gap-2 text-sm font-bold text-emerald-700"><span className="h-2 w-2 rounded-full bg-emerald-500" /> System betriebsbereit</p></div><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Campingplätze" value="1" note="Aktiver Mandant" icon={<TentTree />} /><Metric label="Stationen" value={String(stationCount)} note="Alle veröffentlicht" icon={<MapPinned />} /><Metric label="Besuche heute" value="—" note="Tracking deaktiviert" icon={<Activity />} /><Metric label="Admins" value="1" note="Plattformweit" icon={<Users />} /></div><div className="mt-8 grid gap-6 xl:grid-cols-[1.4fr_1fr]"><section className="rounded-2xl bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><h3 className="font-display text-2xl">Dein Campingplatz</h3><button onClick={() => onNavigate("tenants")} className="text-sm font-bold text-[#286551]">Verwalten <ChevronRight size={16} className="inline" /></button></div><div className="mt-5 flex items-center gap-4 rounded-2xl bg-[#eff3ec] p-4"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#195f4c] font-display text-2xl text-white">{tenant.logoMark}</span><div><p className="font-bold">{tenant.name}</p><p className="text-sm text-[#1b302a]/50">{tenant.slug}.app-domain.de</p></div><span className="ml-auto rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-700">Aktiv</span></div></section><section className="rounded-2xl bg-[#173c32] p-6 text-white"><CircleAlert className="text-[#e8b65f]" /><h3 className="mt-5 font-display text-2xl">Vor dem Livegang</h3><p className="mt-2 text-sm leading-6 text-white/55">Datenbank, Passwort-Hash, Domain und rechtlich geprüfte Texte hinterlegen.</p><button onClick={() => onNavigate("security")} className="mt-5 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-[#173c32]">Checkliste öffnen</button></section></div></div>;
}
  ...
  return <section className="animate-enter overflow-hidden rounded-xl bg-white shadow-sm">
    ...
    <div className="divide-y divide-black/5 lg:hidden">
      {stations.map((station) => <article key={station.id} className="p-4">
        <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words font-bold">{station.name}</h3><p className="mt-1 text-xs text-black/50">{station.categoryId} · {station.openingHours}</p></div><span className="shrink-0 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">{statusLabel[station.status]}</span></div>
...
function TenantSettings({ tenant, saving, onSave }: { tenant: Tenant; saving: boolean; onSave: (tenant: Tenant) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <SettingsCard title="Kartengrundlagen" description="Freie Basiskarte, optionales regionales Luftbild und eigener Platzplan.">
      ...
      <Field label="Kartenstil-URL" value={draft.map.styleUrl} onChange={(styleUrl) => setDraft({ ...draft, map: { ...draft.map, styleUrl } })} />
      <div className="grid gap-4 sm:grid-cols-3">
        <Field label="Mittelpunkt Längengrad" value={String(draft.map.center[0])} onChange={(longitude) => setDraft({ ...draft, map: { ...draft.map, center: [Number(longitude), draft.map.center[1]] } })} />
        <Field label="Mittelpunkt Breitengrad" value={String(draft.map.center[1])} onChange={(latitude) => setDraft({ ...draft, map: { ...draft.map, center: [draft.map.center[0], Number(latitude)] } })} />
        <Field label="Start-Zoom" value={String(draft.map.zoom)} onChange={(zoom) => setDraft({ ...draft, map: { ...draft.map, zoom: Number(zoom) } })} />
      </div>
...
function Modules({ tenant, saving, onSave }: { tenant: Tenant; saving: boolean; onSave: (tenant: Tenant) => void }) { const [draft, setDraft] = useState(tenant); const modules = [["Rundgänge", "tours"], ["Check-ins & QR-Code", "checkins"], ["Veranstaltungen", "events"], ["Feedback", "feedback"], ["Entdecker-Pass", "rewards"], ["Push-Mitteilungen", "push"], ["Belegungs-/Statusanzeigen", "occupancy"], ["Digitale Gästemappe", "guestGuide"]]; return <SettingsCard title="Funktionsmodule" description="Funktionen lassen sich je Campingplatz aktivieren.">{modules.map(([label, id]) => <label key={id} className="flex min-w-0 items-center justify-between gap-4 border-b border-black/5 py-4"><div className="min-w-0"><p className="break-words font-bold">{label}</p><p className="text-xs text-black/45">Für Besucher-App und Verwaltung freischalten</p></div><input type="checkbox" checked={draft.features[id] ?? false} onChange={(event) => setDraft({ ...draft, features: { ...draft.features, [id]: event.target.checked } })} className="h-5 w-5 shrink-0 accent-[#286551]" /></label>)}<Save saving={saving} onClick={() => onSave(draft)} /></SettingsCard>; }
