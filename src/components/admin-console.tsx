...
export function AdminConsole({ tenant, adminEmail }: { tenant: Tenant; adminEmail: string }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#f2f3ef] text-[#1b302a]">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-30 flex w-60 flex-col overflow-y-auto overflow-x-hidden bg-[#173c32] p-4 text-white transition-transform lg:translate-x-0", menuOpen ? "translate-x-0" : "-translate-x-full")}>
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#e8b65f] text-[#173c32]"><Caravan /></span><div><p className="font-display text-xl">Platzguide</p><p className="text-[10px] uppercase tracking-widest text-white/45">Plattform Admin</p></div></div><button className="lg:hidden" onClick={() => setMenuOpen(false)}><X /></button></div>
