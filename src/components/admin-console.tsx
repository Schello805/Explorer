...
function Modules({ tenant, saving, onSave }: { tenant: Tenant; saving: boolean; onSave: (tenant: Tenant) => void }) {
  ...
  return <SettingsCard title="Funktionsmodule" complete description="Funktionen lassen sich je Campingplatz aktivieren. Die aufklappbaren Infos sagen klar, was heute wirklich enthalten ist.">
...
function Integrations({ tenant, saving, platformAdmin, tenantAdminPermissions, onSave }: { tenant: Tenant; saving: boolean; platformAdmin: boolean; tenantAdminPermissions?: { integrations: boolean; analytics: boolean; storage: boolean; backup: boolean }; onSave: (tenant: Tenant) => void }) {
  ...
  return <div className="space-y-6">
    ...
    {canManageAnalytics && <SettingsCard title="Analytics mit Matomo" complete={!draft.tracking.enabled || Boolean(draft.tracking.matomoUrl.trim() && draft.tracking.matomoSiteId.trim())} description="Besucherstatistik je Campingplatz. Tracking startet erst nach Cookie-Einwilligung.">
...
function Billing({ tenant, saving, platformAdmin, onSave }: { tenant: Tenant; saving: boolean; platformAdmin: boolean; onSave: (tenant: Tenant) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <SettingsCard title="Pakete" complete={Boolean(draft.billing.plan && draft.billing.storageLimitMb > 0 && draft.billing.supportResponseHours > 0)} description="Du kannst den Platz kostenlos einrichten und testen. Öffentlich wird die Besucher-App erst mit aktivem Abo oder Superadmin-Freigabe.">
    ...
    <SettingsCard title="Veröffentlichung" complete={draft.billing.status === "active" && draft.billing.publicEnabled} description={platformAdmin ? "Du kannst Status und Freigabe manuell übersteuern." : "Eine Zahlung schaltet nur die Berechtigung frei. Öffentlich wird dein Platz erst nach bewusster Veröffentlichung."}>
...
function Categories({ tenant, saving, onSave }: { tenant: Tenant; saving: boolean; onSave: (tenant: Tenant) => void }) {
  ...
  return <SettingsCard title="Kategorien" complete={draft.categories.length > 0 && draft.categories.every((category) => category.id.trim() && category.name.trim() && category.color.trim())} description="Kategorien strukturieren Filter, Listen und Admin-Importe.">
...
function Media({ tenant, saving, onSave }: { tenant: Tenant; saving: boolean; onSave: (tenant: Tenant) => void }) {
  ...
  return <SettingsCard title="Medienbibliothek" complete={draft.media.every((asset) => asset.title.trim() && asset.url.trim())} description="Bilder, PDFs und kurze Videos zentral verwalten.">
...
function Profile({ tenant, adminEmail }: { tenant: Tenant; adminEmail: string }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-2">
    <SettingsCard title="Profil & Zugang" complete={Boolean(email.trim())} description="Ändere deine Login-E-Mail oder setze ein neues Passwort. Zur Sicherheit ist immer dein aktuelles Passwort nötig.">
...
function Security() { return <div className="min-w-0"><SettingsCard title="Sicherheitsstatus" complete description="Zentrale Schutzmaßnahmen für die Plattform.">{["Rollenmodell für Plattform und Mandanten", "HTTP-only Session-Cookie", "Tenant-Kontext pro Anfrage", "Datenbank-RLS aktiv", "Sicherheitsheader aktiv", "Rate-Limits für Login, Feedback, Check-ins und Uploads", "Datenschutz-Export und Löschanfrage"].map((item) => <p key={item} className="flex min-w-0 items-center gap-3 border-b border-black/5 py-3 text-sm"><ShieldCheck size={18} className="shrink-0 text-emerald-600" /><span className="min-w-0 break-words">{item}</span></p>)}</SettingsCard></div>; }
  ...
  return <div className="fixed inset-0 z-50 flex justify-end overflow-hidden bg-black/35">
    <form onSubmit={(event) => { event.preventDefault(); onSave(draft); }} className="h-full w-full min-w-0 max-w-[min(42rem,100vw)] overflow-x-hidden overflow-y-auto bg-white p-4 shadow-2xl sm:p-6">
      ...
      <div className="mt-8 min-w-0 space-y-5">
        ...
        <label className="flex items-start gap-3 rounded-xl bg-[#f7f7f4] p-4 text-sm font-bold"><input title="Aktiviert diese Station für Besucher. Deaktiviert bleibt sie als Vorlage in der Verwaltung." type="checkbox" checked={!draft.isTemplate} onChange={(event) => setDraft({ ...draft, isTemplate: !event.target.checked })} className="mt-0.5 h-5 w-5 accent-[#286551]" /><span className="inline-flex items-center gap-1.5">In Besucher-App anzeigen<HelpBubble text="Aktiviert diese Station für Besucher. Deaktiviert bleibt sie als Vorlage in der Verwaltung." /></span></label>
      </div>
