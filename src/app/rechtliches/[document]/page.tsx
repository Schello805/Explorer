...
const titles = { impressum: "Impressum", datenschutz: "Datenschutz", cookies: "Cookie-Hinweise", agb: "Allgemeine Nutzungsbedingungen" };
...
export default async function LegalPage({ params }: { params: Promise<{ document: string }> }) {
  ...
  const tenant = await getTenant().catch(() => ({
    ...tenantDefaults,
    name: "Platzguide",
    tagline: "Mobile Campingplatz-App",
    contact: tenantDefaults.contact
  }));
  const key = document as keyof typeof titles;
  const content = key === "impressum" ? tenant.legal.imprint : key === "datenschutz" ? tenant.legal.privacy : key === "cookies" ? tenant.legal.cookies : tenant.legal.terms;
